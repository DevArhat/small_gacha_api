...
class Endfield(Game):
    def run_simulation(self, target_rank):
        ...
        stats = {"game": "Endfield", "total_pulls": 0, "pickup_6": 0, "other_6": 0, 
                 "5_star": 0, "4_star": 0, "log": []}
        ...
        while stats["pickup_6"] < target_copies:
            ...
            # 240회 보너스 (돌파권) 체크
            if stats["total_pulls"] == 240:
                stats["pickup_6"] += 1
                stats["log"].append(f"[Pull {stats['total_pulls']}] 240 Pull Bonus Token Obtained (+1 Copy equivalent)")
            ...
            if guarantee_120_counter == 120 and not used_120_guarantee:
                ...
                stats["log"].append(f"[Pull {stats['total_pulls']}] 120-Pull Hard Guarantee Triggered!")
                continue
            ...
            if pity_6 == 80 or rnd < rate_6:
                ...
                if random.choice([True, False]):
                    ...
                    # 픽업 획득 시 120 카운터 초기화
                    guarantee_120_counter = 0 
                    stats["log"].append(f"[Pull {stats['total_pulls']}] 6★ PICKUP Obtained (Copies: {stats['pickup_6']})")
                else:
                    stats["other_6"] += 1
                    # 픽뚫 시에는 120 카운터 유지됨
                    stats["log"].append(f"[Pull {stats['total_pulls']}] 6★ Standard Character (Spook)")
                continue
